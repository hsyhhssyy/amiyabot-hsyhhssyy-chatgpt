...
**增加了拦截其他插件的范围，具体请看下面的新章节“拦截其他插件”**

**兔兔新版本修复了wait函数的优先级问题，如果遇到安装了本插件以后，某些功能无法响应的问题，请升级到新版兔兔。**

**3.1.0版是最后一个用消息驱动的版本，从下一个版本起，兔兔将不再通过消息驱动，但是表现行为应该没什么区别**

这一版精简了大量的参数，现在会根据群内消息密集程度动态控制自己的插话概率，调整为期望每1分钟插一句话，但是保留连续回复和连续不回复的情况。
下个版本将会更加精简参数，减少配置复杂度，并避免出现诸如兔兔连续说话，兔兔说的话慢半拍等情况。
如果您升级了这一版以后，觉得兔兔话少了，您可以先退回上一个版本。
...
新增一个关键词`ChatGPT请问`，当一句话以`ChatGPT请问`开头时（不需要at兔兔或者添加前缀），将会调用原始ChatGPT会话并且不会触发其他的附加能力，该功能不支持连续对话和上下文。
...
注意，可以用`User:用户QQ号`的形式为私聊创建配置文件，否则私聊默认使用全局配置文件。
