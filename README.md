...
增加了一个“深度角色扮演”选项，具体功能请看下面"深度角色扮演功能"章节的描述。
...
新增配置“阿米娅正在思考中”，默认关闭，打开该配置项，兔兔才会回复“阿米娅正在思考中”。因为现版本下兔兔的响应速度已经很快了，不需要刻意去等待，因此将其改为可选。

2.2版本起，你可以私聊兔兔触发本插件，不过你仍然需要使用回复功能来组织对话。

# 深度角色扮演模式

打开该开关后，下列配置项将不再有意义：“预定义上下文”、“阿米娅正在思考中”、“停止词检测”

打开后，兔兔会随机回复群里的对话，不需要前导词，具体有两种模式：
1. 在某人说了一句超过4个字的话后，超过1分钟没人回复，兔兔会有10%的几率接上这句话。
2. 如果在30秒内，有连续10个人说话，兔兔会通过AI判断一下这5句话是否属于同一个话题，如果是，那么他也会就这个话题回复一句。

现在你仍然可以通过前导词或群内at的形式发起和兔兔的对话，但是现在不再需要回复兔兔的话。
1. 在兔兔回复以后的30秒内，同一个QQ用户直接说话，兔兔就会把他当成是回复，从而继续接话。
2. 其他用户可以选择在60秒内回复兔兔的消息从而加入对话，一旦回复一个对话以后，兔兔就会将该用户加入这个对话之中，之后30秒内，同一个QQ用户说话，不需要回复兔兔，兔兔也会继续接话。
