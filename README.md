...
如果你也想要添加一个助手，请阅读下面的 助手模式 章节。

### 4.1.3

为了适配官兔，经典模式被重做，主要变化是：
1. 强制保持Context，为10句历史对话
2. 支持私聊
3. 修复了一些Prompt问题，优化他在ERNIE-Speed下的表现。（ERNIE-Speed是免费模型）
...
新增一个开关：AI视觉，打开以后，你还需要在下面的·AI视觉模型·配置项中选择一个支持AI视觉的模型。目前仅'gpt-4-vision-preview','gpt-4o'等模型支持，具体请看OpenAI的官方文档。

![Alt text](https://raw.githubusercontent.com/hsyhhssyy/amiyabot-hsyhhssyy-chatgpt/master/images/image-1.png)


打开以后，如果这次对兔兔的提问包含图片，则会转而使用AI视觉模型。否则还是使用原来的模型。在经典模式和ChatGPT请问模式中，你必须要让图片和文本是同一条消息发出才行，在手机上可能不太好操作。在角色扮演模式下，则没有这个限制，你可以先发图，再提问。
...
* `predef_context` 
    - 2.0新增的的配置项目。你可以在其中添加多个先导内容，这些内容会确保每次请求都包含，不会随着对话刷新掉。
    - 你可以通过这些句子给兔兔设置一个角色扮演场景。（注意，要不要遵守还得看AI怎么想）。
    - 如果想不提供任何先导词，请设置一个空的先导词列表。

* `stop_words` 
    - 因为AI并不会严格遵守先导词的规定，因此有时候他还是会回复一些诸如：“作为一个人工智能助手，我XXXX”等的语句，您可以在这里设置一系列字符串，只要系统检测到兔兔的回复里包含任意一个给定的字符串，就会转而回复：“很抱歉博士，但是我不能回答您的这个问题。是否可以请博士换一个问题呢？”，注意不要填写太过于泛用的词汇，防止正常的内容被拦截。
...
助手模式和经典模式很类似，只不过是使用大语言模型库中的Assistant，而不是直接调用API。也因此，上下文和提示词都由助手API进行维护。

如果你也想要创建一个Amiya助手，你可以使用[百度千帆AppBuilder](https://console.bce.baidu.com/ai_apaas/appCenter)。在他的控制台创建一个应用。

在右侧的模型选择下拉框中选择Speed系列模型（这些模型是免费的，但是很笨）。
在左下角的角色指令中，填入本插件目录下的 /templates/deep-cosplay/amiya-template-assistant.txt 的全部内容。这是一份兔兔角色扮演的身份指示范例。

![范例](images/image3.png)

保存该助手，然后将他的app_id以及你的key，填写到大语言模型调用库中。

配置完成后重启兔兔，再在本插件的配置中的默认助手选框，选择你刚刚添加的助手。

现在助手模式就可以使用了。如果你还想要其他进阶能力比如知识库等，就要你自行探索了。
...
新模式锐意开发中，如果你有什么好点子都可以来Github Issue中提出。
...
| 4.2.2  | 角色扮演模式现在可以使用助手而不是直接调用Api, 移除了对statistics库的依赖 |
